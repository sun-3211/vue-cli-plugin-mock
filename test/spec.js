...
let expressMock = require('../mock/index');

let entry = path.resolve(__dirname, './fixtures/entry.js');
...
describe('mock api', function () {
    ...
    it('should skip not match', function (done) {
        let app = express().use(expressMock({entry: entry})).use(function (req, res, next) {
            res.end('is skiped');
        });
    ...
    it('should can mock object vlaue', function (done) {
        let app = express().use(expressMock({entry: entry}));
    ...
    it('should can mock with params', function (done) {
        ...
        request(app).get('/api/userinfo/' + id).expect(200, {id: id, username: 'kenny',}, done);
    });
    it('should can mock with multiple params', function (done) {
        ...
        request(app).get('/api/user/list/' + id + '/' + type).expect(200, {id: id, type: type}, done);
    });
    it('should can accept POST params', function (done) {
        let app = express().use(expressMock({entry: entry}));
        request(app).post('/api/login/account').send({username: 'admin', password: '888888'}).expect(200, {
            status: 'ok',
            code: 0,
            token: 'sdfsdfsdfdsf',
            data: {
